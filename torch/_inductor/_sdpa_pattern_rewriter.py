...
_scaled_dot_product_attention_impl = None # TODO: Set this to torch.nn.functional.scaled_dot_product_attention by default, once that function gets a scale parameter ( planned for 2.1 )

def set_scaled_dot_product_attention_impl(scaled_dot_product_attention_impl : Callable[[Tensor,Tensor,Tensor,Optional[Tensor],float,bool,float], Tensor]) -> None:
    """
...
def fuse_scaled_dot_product_attention(gm: torch.fx.GraphModule):
    ...
    if _scaled_dot_product_attention_impl is None:
        log.warning("fuse_scaled_dot_product_attention(gm) called, without calling set_scaled_dot_product_attention_impl() first. Graph replacements will be dysfunctional.")
